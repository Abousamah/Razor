    ...
    public class BuildServerTestFixture : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            using (var cts = new CancellationTokenSource(_defaultShutdownTimeout))
            {
                ...
                var application = new Application(cts.Token, Mock.Of<ExtensionAssemblyLoader>(), Mock.Of<ExtensionDependencyChecker>(), (path, properties) => MetadataReference.CreateFromFile(path, properties));
